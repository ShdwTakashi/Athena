...
const { checkAuth, checkConnect, getSecrets, getSecretsST } = require("../api");
...
const node = async ({ args }) => {
	...
	let secrets;
	try {
		const context = readFile(".env.infisical");
		const credentials = getCredentials({ host: KEYS_HOST });

		console.log("⬇️  Pulling secrets...");

		if (process.env?.INFISICAL_TOKEN) {
			secrets = await getSecretsST({ 
				workspaceId: context.split(',')[0], 
				environment: context.split(',')[1] || args[0],
			});
		} else {
